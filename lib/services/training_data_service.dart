...
class TrainingDataService {
  ...
  static Future<void> storeInteraction({
    ...
    final interaction = {
      ...
      'user_message': LLMConfig.ANONYMIZE_TRAINING_DATA
          ? _anonymizeMessage(userMessage)
          : userMessage,
  ...
  static Future<List<Map<String, dynamic>>> exportTrainingData({
    ...
    for (var interaction in interactions) {
      ...
      if (interaction['user_rating'] != null &&
          interaction['user_rating'] < minRating) continue;
  ...
  static Future<File> exportToJSONL({
    ...
    final jsonlContent =
        trainingData.map((data) => jsonEncode(data)).join('\n');
  ...
  static Future<Map<String, dynamic>> getTrainingStats() async {
    ...
    final ratedInteractions =
        interactions.where((i) => i['user_rating'] != null).length;
    final highRatedInteractions = interactions
        .where((i) => i['user_rating'] != null && i['user_rating'] >= 4)
        .length;
    ...
    return {
      ...
      'rating_percentage': totalInteractions > 0
          ? (ratedInteractions / totalInteractions) * 100
          : 0,
      'top_topics': topicCounts.entries.toList()
        ..sort((a, b) => b.value.compareTo(a.value))
        ..take(10).map((e) => {'topic': e.key, 'count': e.value}).toList(),
      'ready_for_training':
          highRatedInteractions >= 100, // Minimum recommended for fine-tuning
    };
  }
  ...
  static Future<void> cleanupOldData({int keepLastDays = 90}) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final encodedInteractions =
        filteredInteractions.map((i) => jsonEncode(i)).toList();
    await prefs.setStringList(_interactionsKey, encodedInteractions);
  }

  // Private helper methods
  static Future<void> _storeInteractionLocally(
  ...
  static Future<void> _updateInteractionRating(
      String interactionId, int rating) async {
    final interactions = await _getStoredInteractions();
  ...
  static String _anonymizeMessage(String message) {
    ...
    anonymized =
        anonymized.replaceAll(RegExp(r'\b\d{1,2}/\d{1,2}/\d{4}\b'), '[DATE]');
    anonymized =
        anonymized.replaceAll(RegExp(r'\b\d{4}-\d{2}-\d{2}\b'), '[DATE]');
    anonymized = anonymized.replaceAll(
        RegExp(r'\b\d{1,2}\s*years?\s*old\b'), '[AGE] years old');
    anonymized =
        anonymized.replaceAll(RegExp(r"\bI'm\s+\d{1,2}\b"), "I'm [AGE]");
